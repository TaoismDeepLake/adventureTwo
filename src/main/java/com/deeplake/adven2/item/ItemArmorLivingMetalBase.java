package com.deeplake.adven2.item;

import com.google.common.collect.Multimap;
import com.deeplake.adven2.util.CommonDef;
import com.deeplake.adven2.util.CommonFunctions;
import com.deeplake.adven2.util.IHasModel;
import com.deeplake.adven2.util.NBTStrDef.IDLNBTDef;
		...
		if (!worldIn.isRemote)
        {
            if (((float)stack.getItemDamage() / stack.getMaxDamage()) >= 0.9f)
            {
                if (worldIn.getWorldTime() % CommonDef.TICK_PER_SECOND == 0 && !IDLNBTUtil.GetBoolean(stack, IDLNBTDef.BIOMETAL_WARNED, false)) {
                    ...
                    CommonFunctions.SendMsgToPlayer((EntityPlayerMP) player, "adven2.msg.low_dura", stack.getDisplayName());
                }
            }
            else {
...
//
//        //tooltip.add(I18n.format("adven2.gua_enhance_total.desc", IDLSkillNBT.GetGuaEnhanceTotal(stack)));
